...
- Fallback-классификация спорных сайтов поддерживает 2 транспорта: `SITE_CLASSIFICATION_LLM_PROVIDER=openai` для прямого вызова OpenAI из текущего контейнера и `SITE_CLASSIFICATION_LLM_PROVIDER=gateway` для вызова внешнего LLM gateway-сервиса в разрешённой юрисдикции. В gateway-режиме основной проект формирует тот же payload для `chat/completions`, что и при прямом вызове OpenAI: system prompt, user prompt и `response_format` собираются в `app/modules/serp_ingest.py`, а внешний сервис только проксирует этот payload в OpenAI и возвращает raw JSON-ответ. Это сохраняет единственный источник prompt-логики в основном проекте и убирает расхождение между серверами; контракт relay-сервиса описан в `docs/llm_gateway_service_spec.txt`.
