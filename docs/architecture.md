...
- В `companies` сохраняются только уникальные домены релевантных сайтов; тип сущности хранится в `industry`, город-источник из Google Sheets хранится в `region`, а фактически определённый по сайту/LLM город сохраняется в `attributes.detected_city`. После enrichment в таблице также обновляются `primary_email`, `primary_email_status` и `primary_email_note`, чтобы основной адрес и причина его отсутствия были доступны без отдельного join к `contacts`.
